...
- **New ML features discovered** → FeatureEngineerAgent reads Gold layer, asks Claude for 5 new features, validates each in an 8-gate sandbox (including distribution sanity, variance, and novelty checks), writes them — automatically merged into the next training run
...
| **Feature Engineer** | Claude Haiku | 15 min | Reads Gold layer. Asks Claude for new features. Validates each via 8-gate sandbox (syntax, NaN, distribution, variance, novelty). Writes to `computed_features/`. Triggers ML retraining. |
...
3. **8-gate code validation** — features generated by Claude pass: syntax → execution sandbox → NaN check → distribution sanity (no infinities) → variance check (no constants) → novelty check (no near-duplicate columns) → schema check → timeout kill. Never written without all 8 gates passing.
...
### HMAC-Signed Agent Corrections

Every correction the orchestrator issues to a sub-agent is signed with HMAC-SHA256. Sub-agents verify the signature before applying it — an unsigned or tampered correction is logged and silently dropped.

```
Orchestrator                            Sub-Agent
    │                                       │
    ├── sign_correction(message)            │
    │   → HMAC-SHA256(CORRECTION_HMAC_KEY)  │
    │                                       │
    ├── state.write_correction(sig=sig) ───►DB
    │                                       │
    │                             check_for_corrections()
    │                             → get_pending_corrections_raw()
    │                             → verify_correction(message, sig)
    │                             → apply_correction() only if verified
```

Set `CORRECTION_HMAC_KEY` in `.env` to enable. Dev mode (key absent) accepts all corrections as before.

### Prompt Injection Defense

`agents/security.sanitize_for_prompt()` is available on every agent via `self.sanitize_external_input()`. It strips null bytes, truncates to a safe length, and logs a WARNING on any of 8 known injection patterns (instruction override, role override, XML tag injection, model delimiter injection, jailbreak attempts, etc.).

Call it on any external text before embedding it in an LLM prompt:
```python
clean = self.sanitize_external_input(csv_row["supplier_note"], field_name="supplier_note")
# then use: f"<supplier_note>{clean}</supplier_note>" in the prompt
```

### Immutable Hash-Chained Audit Log

`agent_audit_log_v2` stores every agent decision with a SHA-256 hash chain:

```
row N-1: row_hash = sha256("orchestrator|CORRECTION_ISSUED|...|{prev_hash_N-2}")
row N  : prev_hash = row_hash of row N-1
         row_hash  = sha256("feature_engineer|FEATURES_GENERATED|...|{prev_hash}")
```

Written with `state.write_audit_immutable()`. The SELECT + INSERT are atomically wrapped in a serializable transaction so the chain cannot have gaps under concurrent writes. The legacy `agent_audit_log` table is preserved for backwards compatibility.

### Feature Poisoning Prevention (8-Gate Validation)

When the FeatureEngineerAgent receives Claude's feature suggestions, each candidate passes through 8 gates before being written to `computed_features/`:

| Gate | Check | Rejects |
|------|-------|---------|
| 1 | Syntax + CodeExecutor validation | Code that doesn't parse or crashes |
| 2 | NaN ratio < 5% on 50-row sample | High-null features |
| 6 | No infinite / abs(value) > 1e10 | Division-by-zero, overflow errors |
| 7 | std > 0.001 on 200 rows | Constant or near-constant features |
| 8 | Max Pearson \|r\| < 0.99 with all existing columns | Near-duplicate features |

Gates 6-8 add ~60-90s per feature in the worst case. Since the agent runs every 15 minutes this is acceptable.

### Human Escalation Workflow

When the orchestrator determines human action is required (`human_intervention_needed=True`), it writes to the `human_escalations` table. Operators can view and resolve these via the REST API:

```bash
# List open escalations
GET /escalations                  → { "escalations": [...], "total": N }

# Get a specific escalation
GET /escalations/{id}

# Resolve (acknowledge + close)
POST /escalations/{id}/resolve    body: { "resolved_by": "ops-team" }

# Summary stats
GET /escalations/stats/summary    → { "open_count", "resolved_count", "avg_resolve_minutes" }
```

Escalation severity follows the same 1/2/3+ agent degradation rules as the orchestrator.

### Governance Tables

Six new PostgreSQL tables support the governance layer:

| Table | Purpose |
|-------|---------|
| `agent_audit_log_v2` | Immutable hash-chained audit log for all agent decisions |
| `human_escalations` | Corrections requiring human action — OPEN/RESOLVED lifecycle |
| `decision_provenance` | Full reasoning chain: trigger → data sources → LLM hashes → action |
| `correction_outcomes` | Effectiveness measured 15 min after each orchestrator correction |
| `data_classifications` | Per-column sensitivity labels (PUBLIC / INTERNAL / CONFIDENTIAL / PII) |
| `agent_corrections` | Extended with `signature TEXT` column for HMAC verification |

All tables are created idempotently via `state.ensure_tables()` at agent startup. The `signature` column is added to the existing `agent_corrections` table via `ALTER TABLE ... ADD COLUMN IF NOT EXISTS` — safe on existing deployments.
...
| `CORRECTION_HMAC_KEY` | Secret key for HMAC-signed agent corrections. When set, only the orchestrator can issue valid corrections. Leave empty in dev. | — |
